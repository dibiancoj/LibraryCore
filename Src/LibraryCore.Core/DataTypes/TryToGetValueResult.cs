...
public record TryToGetValueResult<T>(bool ItemWasFound, T? ItemValueIfFound)
{
    /// <summary>
    /// Tries to get the item from the source. Will return the result if it was found.
    /// </summary>
    /// <param name="itemValueIfFound">The item from source if found</param>
    /// <returns>If the item was found in session (yes / no)</returns>
    public bool GetValueIfFound([NotNullWhen(true)] out T? itemValueIfFound)
    {
        itemValueIfFound = ItemValueIfFound;
        return ItemWasFound;
    }
}
