...
public static class JsonNetUtilities
{
    ...
    public static async Task<JsonToStreamResult> SerializeToStreamAsync<T>(T? modelToSerialize, JsonSerializer jsonSerializer)
    {
        ...
        await result.JsonTextWriterToUse.FlushAsync();
        await result.StreamWriterToUse.FlushAsync();

        return result;
    }

    public static async Task<byte[]> SerializeToUtf8BytesAsync<T>(T? modelToSerialize, JsonSerializer jsonSerializer) => (await SerializeToStreamAsync(modelToSerialize, jsonSerializer)).ToByteArray();
