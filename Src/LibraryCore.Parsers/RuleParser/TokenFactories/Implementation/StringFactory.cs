...
public class StringFactory : ITokenFactory
{
                              ...
                              SchemaModel schema)
    {
        ...
        while (stringReader.HasMoreCharacters() && stringReader.PeekCharacter() != TokenIdentifier)
        {
            ...
            if (currentCharacter == '{')
            {
                subTokens.Add(ResolveInnerTokens(stringReader, tokenFactoryProvider, ruleParserEngine, schema));
                                             ...
                                             SchemaModel schema)
    {
        while (stringReader.HasMoreCharacters() && stringReader.PeekCharacter() != '}')
        {
            ...
            var token = factoryFound.CreateToken(characterRead, stringReader, tokenFactoryProvider, ruleParserEngine, schema);
